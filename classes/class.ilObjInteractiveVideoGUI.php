...
class ilObjInteractiveVideoGUI extends ilObjectPluginGUI implements ilDesktopItemHandling
{
		...
		switch($next_class)
		{
			...
			default:
				...
				switch($cmd)
				{
					...
					case 'removeFromDesk':
					case 'showContent':
						if(in_array($cmd, ['addToDesk', 'removeFromDesk']))
						{
							$cmd .= 'Object';
						}
	...
	protected function buildContentTemplate(bool $light_version = false): ilTemplate
	{
		...
		$video_tpl->setVariable('CONFIG', $this->initPlayerConfig($player_id, $this->object->getSourceId()));
	...
	protected function initPlayerConfig($player_id, $video_type, bool $edit_screen = false): string
	{
		...
		$config_tpl->setVariable('VIDEO_FINISHED_POST_URL', $this->ctrl->getLinkTargetByClass(['ilRepositoryGUI', 'ilObjInteractiveVideoGUI'], 'postVideoFinishedPerAjax', '', true));
		$config_tpl->setVariable('VIDEO_STARTED_POST_URL', $this->ctrl->getLinkTargetByClass(['ilRepositoryGUI', 'ilObjInteractiveVideoGUI'], 'postVideoStartedPerAjax', '', true));
...
//		$repeatCorrect = 1;  //switch this: 1 --> repeat questons disregarding status, 0 --> only repeat incorrectly answered
    ...
    public function setSubTabs(string $a_tab): void
	{
        ...
        if ($a_tab == 'editComments') {
            if ($ilAccess->checkAccess('write', '', $this->object->getRefId())) {
                $ilTabs->addSubTab('editComments', $plugin->txt('questions_comments_sub_tab'),
                    $this->ctrl->getLinkTarget($this, 'editComments'));
            }
...
#region COMMENTS
    ...
    public function editComments(int $current_time = 0): void
	{
		...
		$video_tpl->setVariable('POST_COMMENT_URL', $this->ctrl->getLinkTarget($this, 'postTutorComment', ''));
    ...
    private function getChapterFomValues(int $comment_id = 0)
	{
        ...
        if($comment_id === 0) {
            ...
            } elseif($comment_id === 0  || $comment_id === "null")  {
                return [];
            } else {
                $comment_id = 0;
            }
        ...
        if ($post->has('comment_title')) {
            $comment_title = $post->retrieve('comment_title', $this->refinery->kindlyTo()->string());
        } elseif(isset($comment_data['comment_title'])) {
        ...
        if ($post->has('comment_text')) {
            $comment_text = $post->retrieve('comment_text', $this->refinery->kindlyTo()->string());
        } elseif(isset($comment_data['comment_text'])) {
        ...
        if(isset($comment_data['comment_time_end'])) {
            $values['comment_time_end']	= $comment_data['comment_time_end'];
        }

		$values['comment_text']		= $comment_text;
        if(isset($comment_data['is_interactive'])) {
            $values['is_interactive']	= $comment_data['is_interactive'];
        }

		$values['comment_title']	= $comment_title;
        if(isset($comment_data['comment_tags'])) {
            $values['comment_tags']		= $comment_data['comment_tags'];
        }

        if(isset($comment_data['is_private'])) {
            $values['is_private']		= $comment_data['is_private'];
        }

        if(isset($comment_data['is_table_of_content'])) {
            $values['is_table_of_content'] = $comment_data['is_table_of_content'];
        }
	...
	private function getQuestionFormValues(int $comment_id = 0): array
	{
        ...
        if(isset($question_data['question_data'])) {
            ...
            $values['jump_correct_ts']			= ilInteractiveVideoTimePicker::getTimeStringFromSeconds((int) $question_data['question_data']['jump_correct_ts']);
            ...
            $values['jump_wrong_ts']			= ilInteractiveVideoTimePicker::getTimeStringFromSeconds((int) $question_data['question_data']['jump_wrong_ts']);
    ...
    public function confirmUpdateQuestion(): void
	{
        ...
        if($post->has('question_text')) {
            $question_text = $post->retrieve('question_text', $this->refinery->kindlyTo()->string());
        }
    ...
    private function existingAnswersForAnswerOptions() {
        /**
         * @var $tpl    ilTemplate
         * @var $ilTabs ilTabsGUI
         */
	...
	public function getAnswerDefinitionsJSON(): string
	{
		...
		{
			$question->setVariable('JSON', json_encode([]));
			$question->setVariable('QUESTION_TYPE', 0);
		}
...
#region AJAX
    ...
    public function generateThumbnailsFromSourcePerAjax(): void
	{
		...
		if($source->isFileBased())
		{
			...
			{
				$tpl_json->setVariable('JSON', json_encode(['error' => $e->getMessage()]));
				$tpl_json->show("DEFAULT", false, true);
			}

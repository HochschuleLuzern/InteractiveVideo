...
class SimpleChoiceQuestionsTableGUI extends ilTable2GUI
{
	...
	public function __construct($a_parent_obj, $a_parent_cmd)
	{
		...
		$this->setRowTemplate('tpl.row_answers.html', $a_parent_obj->getPlugin()->getDirectory());
		$this->addColumn('', 'user_id',  '1px', true);

		$this->addColumn($this->lng->txt('name'), 'name');
		$this->addColumn($a_parent_obj->getPlugin()->txt('answered'), 'answered');
		$this->addColumn($a_parent_obj->getPlugin()->txt('neutral_questions'), 'neutral_questions');
		$this->addColumn($a_parent_obj->getPlugin()->txt('correct'), 'correct');
